...
UNION 
SELECT ss.klient_id FROM `szanseSprzedazy` ss WHERE ss.id_procesu IN (15,16,18,21,23,38,40,50,53,54,55,62,65) AND ss.status IN ('otwarta')
UNION 
...
UNION 
SELECT ss.klient_id FROM `szanseSprzedazy` ss WHERE ss.id_procesu IN (15,16,18,21,23,38,40,50,53,54,55,62,65) 
        ...
        if ($result = mysqli_store_result($link)) {
            ...
            while ($w = mysqli_fetch_assoc($result)) {
                $data[$i][] = array(
                    ...
                    'email'     => $w['email'],
                    'level'     => $w['level'],
                    'nazwa_produktu' => $w['nazwa_produktu'] ? $w['nazwa_produktu'] : ''
                );
            }
            $result->free();
        }
